...
enum RFSTATE {
    ...
    RFSTATE_RX,
    RFSTATE_TX,
    RFSTATE_NOT_READY, // Idle but sendPacket and getPacket do not try to use the chip and return false
    RFSTATE_PWR_DOWN, // low power mode, otherwise like RFSTATE_DISABLED
    RFSTATE_TRANSMIT,
    RFSTATE_WOR
};
...
#define setPower(power)             writeReg(CC1101_PATABLE, power)
// PATABLE values
...
class mCC1101
{
    ...
    public:
        ...
        // a "const" is added
        void writeBurstReg(byte regAddr, const byte* buffer, byte len);
        ...
        bool sendPacket(const byte* pkt, const byte len);
        uint32_t sendBurstPacket(const byte* packet, const byte length, const uint32_t msec);
