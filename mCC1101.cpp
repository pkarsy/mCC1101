 ...
 */
void mCC1101::writeReg(byte regAddr, byte value)
{
	cc1101_Select();                      // Select CC1101
	wait_Miso();                          // Wait until MISO goes low
 ...
 * 'len'    Data length
 */
// TODO  byte* buffer -> const  byte* buffer
void mCC1101::writeBurstReg(byte regAddr, const byte* buffer, byte len)
{
	byte addr, i;
 ...
 * 'cmd'    Command strobe
 */
void mCC1101::cmdStrobe(byte cmd)
{
	cc1101_Select();                      // Select CC1101
	wait_Miso();                          // Wait until MISO goes low
 ...
 */
byte mCC1101::readReg(byte regAddr, byte regType)
{
	byte addr, val;
 ...
 * 'len'    Data length
 */
void mCC1101::readBurstReg(byte * buffer, byte regAddr, byte len)
{
	byte addr, i;
 ...
 */
void mCC1101::setDefaultRegs(void)
{
	...
	writeReg(CC1101_PATABLE, PA_LongDistance);
}
...
void mCC1101::init()
{
	...
	// itan comment apo tin arxi
	// Configure PATABLE
	// writeBurstReg(CC1101_PATABLE, (byte*)paTable, 8);

	// tha to valoume ekso apo to init
	// writeReg(CC1101_PATABLE, power);
	//enabled = true;
	// rfState = _rfstate;
 ...
 */
void mCC1101::setSyncWord(uint8_t syncH, uint8_t syncL)
{
 ...
 * 'save' If TRUE, save parameter in EEPROM
 */
void mCC1101::setDevAddress(byte addr)
{
	//if (devAddress != addr)
	//{
 ...
 * 'save' If TRUE, save parameter in EEPROM
 */
void mCC1101::setChannel(byte chnl)
{
	//if (channel != chnl)
	//{
 ...
 */
void mCC1101::setCarrierFreq(byte freq)
{
	switch(freq)
	{
		case CFREQ_915:
 ...
 */
//void mCC1101::setRegsFromEeprom(void)
//{
 ...
 */
void mCC1101::setPowerDownState()
{
	//enabled = false;
	//rfState = RFSTATE_NOT_READY;
	// Comming from RX state, we need to enter the IDLE state first
	cmdStrobe(CC1101_SIDLE);

	// diko mou
	cmdStrobe(CC1101_SFRX); // Flush RX buffer
	cmdStrobe(CC1101_SFTX); // Flush TX buffer kainourio enai auto ?
...
// diko mou
void mCC1101::leavePowerDownState()
{
	// Comming from RX state, we need to enter the IDLE state first
	cmdStrobe(CC1101_SIDLE);
 ...
 *    False otherwise
 */
/* boolean mCC1101::sendData(const CCPACKET & packet)
{
	byte marcState;
	bool res = false;
 ...
 */
/* byte mCC1101::receiveData(CCPACKET * packet)
{
	byte val;
...
bool mCC1101::sendPacket(const byte* packet, byte length)
{
	
	//if (not enabled) return false;
	//if (rfState==RFSTATE_NOT_READY) return false;
	//while (rfState==RFSTATE_TX) ; // perimenei na figei to paketo

	//rfState = RFSTATE_TX;

	if (length>CC1101_DATA_LEN) length = CC1101_DATA_LEN;
	byte marcState;
	bool res = false;
	...
	// Set data length at the first position of the TX FIFO
	writeReg(CC1101_TXFIFO,  length);
	// Write data into the TX FIFO
	writeBurstReg(CC1101_TXFIFO, packet, length);
	...
	// dinei xrono sto interrupt na treksei
	__asm__("nop\n\t");
	interrupt = false;
...
// stelnei paketa poli grigora gia na a piasei to WOR, epistrefei afou teleiwsei
uint32_t mCC1101::sendBurstPacket(const byte* packet, const byte length, const uint32_t msec) {
	byte marcState;
	uint32_t counter=0;
	...
	writeReg(CC1101_MCSM1,  0x22); // to tx menei tx
	
	setTxState();

	uint32_t tm = millis();
	while (millis()-tm < msec) {
		counter++;
		//setIdleState();
		//flushTxFifo();
		//Set data length at the first position of the TX FIFO
	...
	setIdleState();       // Enter IDLE state
	// ksanagurizume ton MCSM1 stin arxiki tou timi
	writeReg(CC1101_MCSM1,  CC1101_DEFVAL_MCSM1);
...
byte mCC1101::getPacket(byte* packet)
{
	...
	if (rxBytes & 0x7F && !(rxBytes & 0x80)) {
		// Read data l
		l = readConfigReg(CC1101_RXFIFO);
		// If packet is too long
		if (l > CC1101_DATA_LEN) {
			l = 0;   // Discard packet
		}
		else if (l>0) {
			// Read data packet
			readBurstReg(packet, CC1101_RXFIFO, l);
...
bool mCC1101::print(const __FlashStringHelper* fmt, ...) {
	byte pkt[CC1101_DATA_LEN+1];
	va_list args;
	va_start(args, fmt);
	byte length = vsnprintf_P( (char*)pkt,CC1101_DATA_LEN+1, (const char*)fmt, args );
	va_end(args);
	if (length>CC1101_DATA_LEN) length=CC1101_DATA_LEN;
	bool succ =  sendPacket(pkt,length);
	return succ;
}
